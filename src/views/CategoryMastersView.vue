<template>
    <div class="category-masters">
      <div class="container">
        <div class="search-row">
          <input type="text" v-model="searchQuery" placeholder="Поиск мастера..." class="search-input" />
        </div>
          ...
          <div v-else>
            ...
            <div v-else class="masters-grid">
              <div v-for="master in filteredMasters" :key="master.id" class="master-card" @click="goToMaster(master.id)">
                ...
                <div class="price">от {{ master.price }} ₽</div>
  ...
  const categoryMasters = computed(() => 
    allMasters.value.filter(m => m.category.toLowerCase() === categoryName.toLowerCase())
  )
  
  const filteredMasters = computed(() => {
    return categoryMasters.value.filter(master =>
      master.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      master.service.toLowerCase().includes(searchQuery.value.toLowerCase())
    )
  })
